...
def create_sidebar_filters(df):
    ...
    return departments, locations, remote_filter

def filter_data(df, departments, locations, remote_filter):
    """Apply filters to the dataframe"""
    filtered_df = df[df['department'].isin(departments) & df['office_location'].isin(locations)]
...
def display_charts(filtered_df):
    ...
    with col1:
        ...
        fig1 = px.pie(values=dept_counts.values, names=dept_counts.index, hole=0.4)
        st.plotly_chart(fig1, use_container_width=True)
    
    with col2:
        st.subheader("Average Salary by Department")
        dept_salary = filtered_df.groupby('department')['salary'].mean().sort_values(ascending=True)
        fig2 = px.bar(x=dept_salary.values, y=dept_salary.index, orientation='h')
        fig2.update_layout(xaxis_title="Average Salary", yaxis_title="Department")
    ...
    with col3:
        ...
        fig3 = px.histogram(filtered_df, x='performance', nbins=20)
        fig3.update_layout(xaxis_title="Performance Score", yaxis_title="Count")
        st.plotly_chart(fig3, use_container_width=True)
    
    with col4:
        st.subheader("Employees by Location")
...
def main():
    ...
    departments, locations, remote_filter = create_sidebar_filters(df)
    
    # Filter data
    filtered_df = filter_data(df, departments, locations, remote_filter)
